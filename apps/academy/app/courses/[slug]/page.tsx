...
export default function CourseDetail() {
  ...
  if (!course) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4 p-8">
        ...
        <button onClick={() => router.back()} className="text-sm font-semibold" style={{ color: "var(--ac-primary)" }}>← Quay lại</button>
      </div>
    );
  }
  ...
  return (
    <div className="pb-8">
      <div className="sticky top-0 z-10 flex items-center gap-3 px-4 py-3" style={{ background: "var(--ac-bg)" }}>
        <button onClick={() => router.back()} className="w-8 h-8 flex items-center justify-center rounded-full" style={{ background: "var(--ac-surface)" }}>
          <ArrowLeft size={18} />
        </button>
      ...
      <div className="mx-4 mt-4">
        ...
        <div className="flex gap-3 mt-3 flex-wrap">
          {[
            ...
            <div key={label} className="rounded-lg px-3 py-1.5 text-xs" style={{ background: "#e0e7ff", color: "var(--ac-primary)" }}>
              ...
              return (
                <div key={lesson.id} className="flex items-center gap-3 p-3 rounded-xl"
                  ...
                  <div className="w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0"
                    ...
                    {accessible ? <Play size={14} color="var(--ac-primary)" /> : <Lock size={14} color="var(--ac-muted)" />}
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium truncate">{idx + 1}. {lesson.title}</p>
                    {lesson.duration > 0 && (
                      <p className="text-xs" style={{ color: "var(--ac-muted)" }}>{Math.floor(lesson.duration / 60)} phút</p>
                    )}
                  </div>
                  {lesson.isFree && (
                    <span className="text-xs font-semibold px-2 py-0.5 rounded" style={{ background: "#d1fae5", color: "#065f46" }}>Free</span>
                  )}
                </div>
              );
            })}
