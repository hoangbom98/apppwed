...
export default function Dashboard() {
  ...
  // ── KPI stats ────────────────────────────────────────────────────────────────
  const KPI = [
    { title: 'Tổng users',          value: data?.users?.total ?? 0,                icon: <TeamOutlined />,        color: token.colorPrimary, sub: data?.users?.newToday ? `+${data.users.newToday} hôm nay` : null },
    { title: 'Đăng ký hôm nay',     value: data?.users?.newToday ?? 0,             icon: <UserAddOutlined />,     color: '#06b6d4' },
    ...
    { title: 'Phòng chat mở',       value: data?.activity?.openTickets ?? 0,       icon: <EyeOutlined />,         color: '#8b5cf6' },
  ];
  ...
  return (
    <Spin spinning={isLoading} tip="Đang tải...">
      ...
      <Row gutter={[16, 16]} style={{ marginBottom: 20 }}>
        {KPI.map(k => (
          <Col key={k.title} xs={12} sm={8} xl={3}>
