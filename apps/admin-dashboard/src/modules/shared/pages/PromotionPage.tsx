...
import {
  ...
  Typography, Space, App, Flex, Checkbox,
} from 'antd';
import { PlusOutlined, EditOutlined, TeamOutlined, PoweroffOutlined, GiftOutlined } from '@ant-design/icons';
...
// ── Reward Tiers Sub-panel (học từ ActivityRewardController.php Boyue) ────────
// Mỗi khuyến mãi có thể có nhiều mức thưởng (tiers) theo điều kiện nạp
const REWARD_TYPES   = ['FIXED', 'PERCENTAGE', 'FREE_SPIN', 'ITEM'];
const LIMIT_PERIODS  = ['once', 'daily', 'weekly', 'monthly', 'unlimited'];

function RewardTiersModal({ promoId, promoTitle, onClose }) {
  const { message } = App.useApp();
  const qc = useQueryClient();
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['admin-promo-tiers', promoId],
    queryFn:  () => api.get(`/admin/promotions/${promoId}/tiers`).then(r => r.data),
    enabled:  !!promoId,
    retry:    false, // endpoint optional — silently fail
  });
  const tiers = data?.data ?? data ?? [];

  const addMut = useMutation({
    mutationFn: values => api.post(`/admin/promotions/${promoId}/tiers`, values),
    onSuccess:  () => {
      qc.invalidateQueries({ queryKey: ['admin-promo-tiers', promoId] });
      addForm.resetFields();
      setAdding(false);
      message.success('Đã thêm tier');
    },
    onError: e => message.error(e?.response?.data?.message ?? 'Lỗi'),
  });
  const delMut = useMutation({
    mutationFn: id => api.delete(`/admin/promotions/${promoId}/tiers/${id}`),
    onSuccess:  () => { qc.invalidateQueries({ queryKey: ['admin-promo-tiers', promoId] }); message.success('Đã xoá'); },
    onError:    () => message.error('Lỗi xoá'),
  });

  const tierCols = [
    { title: 'Mức',        dataIndex: 'levelName',     render: v => <Text strong>{v || '—'}</Text> },
    { title: 'Loại thưởng',dataIndex: 'rewardType',    render: v => <Tag>{v}</Tag> },
    { title: 'Số tiền',    dataIndex: 'rewardAmount',  render: v => v ? <Text style={{ color: '#10b981' }}>{fmt(v)}₫</Text> : '—' },
    { title: 'Tỷ lệ',      dataIndex: 'rewardRate',    render: v => v ? `${v}%` : '—' },
    { title: 'Min cược',   dataIndex: 'conditionMin',  render: v => fmt(v) },
    { title: 'Max cược',   dataIndex: 'conditionMax',  render: v => fmt(v) },
    { title: 'Giới hạn',   dataIndex: 'limitTimes',    render: (v, r) => v ? `${v}×/${r.limitPeriod}` : '∞' },
    { title: 'Cần đăng ký',dataIndex: 'needApply',     render: v => v ? <Tag color="warning">Có</Tag> : '—' },
    { title: '', key: 'del', width: 50, render: (_, r) => <Button size="small" danger onClick={() => delMut.mutate(r.id)}>Xoá</Button> },
  ];

  return (
    <Modal
      open={!!promoId}
      title={`Mức thưởng — ${promoTitle}`}
      onCancel={onClose}
      footer={null}
      width={860}
      destroyOnHidden
    >
      <Flex justify="flex-end" style={{ marginBottom: 8 }}>
        <Button type="primary" icon={<PlusOutlined />} size="small" onClick={() => setAdding(true)}>Thêm tier</Button>
      </Flex>
      <Table dataSource={tiers} columns={tierCols} rowKey="id"
        loading={isLoading} size="small" scroll={{ x: 700 }} pagination={false} />

      {/* Add tier form */}
      {adding && (
        <div style={{ marginTop: 16, padding: 16, background: '#f7f8fa', borderRadius: 8 }}>
          <Text strong style={{ display: 'block', marginBottom: 8 }}>Thêm mức thưởng mới</Text>
          <Form form={addForm} layout="inline" onFinish={v => addMut.mutate(v)}>
            <Form.Item name="levelName" label="Tên mức" style={{ marginBottom: 8 }}>
              <Input size="small" placeholder="vd: Mức 1" style={{ width: 100 }} />
            </Form.Item>
            <Form.Item name="rewardType" label="Loại" rules={[{ required: true }]} style={{ marginBottom: 8 }}>
              <Select size="small" options={REWARD_TYPES.map(t => ({ label: t, value: t }))} style={{ width: 110 }} />
            </Form.Item>
            <Form.Item name="rewardAmount" label="Số tiền" style={{ marginBottom: 8 }}>
              <InputNumber size="small" min={0} style={{ width: 110 }} />
            </Form.Item>
            <Form.Item name="rewardRate" label="Tỷ lệ %" style={{ marginBottom: 8 }}>
              <InputNumber size="small" min={0} max={100} step={0.1} style={{ width: 80 }} />
            </Form.Item>
            <Form.Item name="conditionMin" label="Min" style={{ marginBottom: 8 }}>
              <InputNumber size="small" min={0} style={{ width: 100 }} />
            </Form.Item>
            <Form.Item name="conditionMax" label="Max" style={{ marginBottom: 8 }}>
              <InputNumber size="small" min={0} style={{ width: 100 }} />
            </Form.Item>
            <Form.Item name="limitTimes" label="Lần" style={{ marginBottom: 8 }}>
              <InputNumber size="small" min={1} style={{ width: 70 }} />
            </Form.Item>
            <Form.Item name="limitPeriod" label="Kỳ" style={{ marginBottom: 8 }}>
              <Select size="small" options={LIMIT_PERIODS.map(p => ({ label: p, value: p }))} style={{ width: 100 }} />
            </Form.Item>
            <Form.Item name="needApply" valuePropName="checked" label="Cần đăng ký" style={{ marginBottom: 8 }}>
              <Checkbox />
            </Form.Item>
            <Form.Item style={{ marginBottom: 8 }}>
              <Space>
                <Button size="small" type="primary" htmlType="submit" loading={addMut.isPending}>Lưu</Button>
...
export default function PromotionPage() {
    ...
    { title: 'TT', dataIndex: 'status', key: 'status', render: v => <Tag color={STATUS_COLOR[v] ?? 'default'}>{v}</Tag> },
    {
      ...
      render: (_, p) => (
        <Space size={4} wrap>
          ...
          <Button size="small" icon={<GiftOutlined />} onClick={() => setTiers({ id: p.id, title: p.title })}>Mức thưởng</Button>
  ...
  return (
    <div>
      ...
      {viewTiers && (
        <RewardTiersModal promoId={viewTiers.id} promoTitle={viewTiers.title} onClose={() => setTiers(null)} />
      )}
    </div>
  );
}
