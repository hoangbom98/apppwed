...
import { ServiceRegistry }          from './src/third-parties/core/ServiceRegistry';
...
// 1. Load game aggregators from game_db (GSC, Goldgate, TCGaming, Binance…)
// 2. Bootstrap sports public API providers from env (API-Football, TheSportsDB, GNews)
const _registry = ServiceRegistry.getInstance();
_registry.ensureLoaded().catch((err: Error) => {
  logger.warn('[ThirdParty] ServiceRegistry failed to load game aggregators', { error: err.message });
});
_registry.bootstrapSportsProviders();   // env-based sports providers (always safe)
