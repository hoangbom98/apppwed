...
const DOMAIN = process.env.MONITOR_DOMAIN ?? 'tc-gaming.live';
const VPS_IP = process.env.MONITOR_VPS_IP  ?? '104.248.146.203';

// ── Internal HTTP probe ───────────────────────────────────────────────────────

interface ProbeResult { status: number; body: string; ms: number; ssl?: boolean; error?: string; }

function probe(url: string, timeoutMs = 6000): Promise<ProbeResult> {
...
exports.getOnlineStats = async (req, res) => {
  try {
    const sessionSvc = require('../../../shared/services/core/sessionService');
    const projects   = ['hub', 'game', 'trade', 'dating', 'sports'];
    const stats: Record<string, any> = { total: 0, byProject: {} };
...
exports.healthServices = async (req, res) => {
  ...
  // Redis — ping via raw ioredis client
  try {
...
exports.healthAll = async (req, res) => {
  ...
  const [svcProbes, dnsProbes] = await Promise.all([
    Promise.all(ENDPOINTS.map(async svc => {
      const r = await probe(svc.url);
      const status = r.error || r.status === 0 ? 'offline' : r.status < 400 ? 'online' : 'warning';
      return { name: svc.name, url: svc.url, type: svc.type, status, responseTime: r.ms, details: r.error ?? `HTTP ${r.status}` };
    })),
