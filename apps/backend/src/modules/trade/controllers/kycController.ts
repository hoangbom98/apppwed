...
exports.approveKyc = async (req, res) => {
  try {
    ...
    // Get KYC reward from config
    const cfgRow = await req.prisma.systemConfig.findUnique({ where: { key: 'kyc_reward' } });
    const kycReward = parseFloat(cfgRow?.value || '0');
    ...
    await req.prisma.$transaction(async (tx) => {
      ...
      // Credit KYC reward to wallet
      if (kycReward > 0) {
        const wallet = await tx.wallet.findUnique({ where: { userId } });
        const newBal = (wallet ? parseFloat(wallet.balance) : 0) + kycReward;
        await tx.wallet.upsert({
          where:  { userId },
          create: { userId, balance: kycReward, frozen: 0 },
          update: { balance: { increment: kycReward } },
        });
        await tx.transaction.create({
          data: {
            ...
            amount:        kycReward,
            balanceAfter:  newBal,
            note:          `Thưởng xác minh danh tính (KYC)`,
          },
        });
      }
    });
