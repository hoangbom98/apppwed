...
'use strict';
/**
 * savingsVaultController — money market / savings products (余额宝)
 *
 * User routes:  GET /trade/savingsVault/products, POST /trade/savingsVault/invest, GET /trade/savingsVault/my
 * Admin routes: CRUD /trade/admin/savingsVault/products, PUT .../settle, PUT .../renew
 */
...
// GET /trade/savingsVault/products
...
exports.invest = async (req, res) => {
  try {
    ...
    const investment = await req.prisma.$transaction(async (tx: any) => {
      ...
      const inv = await tx.savingsVaultInvestment.create({
      ...
      await tx.transaction.create({
        data: {
          ...
          note:          `Đầu tư SavingsVault: ${product.title}`,
        },
      });
      return inv;
    });
...
exports.adminCreateProduct = async (req, res) => {
  try {
    ...
    const product = await req.prisma.savingsVaultProduct.create({
    ...
    return success(res, product, 'Đã tạo sản phẩm SavingsVault');
  } catch (e: any) { return error(res, e.message, 500); }
};

exports.adminUpdateProduct = async (req, res) => {
  try {
    const product = await req.prisma.savingsVaultProduct.update({
...
exports.adminDeleteProduct = async (req, res) => {
  try {
    await req.prisma.savingsVaultProduct.update({ where: { id: req.params.id }, data: { status: 'deleted' } });
...
// PUT /trade/admin/savingsVault/investments/:id/settle — manual settle
exports.adminSettle = async (req, res) => {
  try {
    const inv = await req.prisma.savingsVaultInvestment.findUnique({
    ...
    await req.prisma.$transaction(async (tx: any) => {
      ...
      await tx.transaction.create({
        data: {
          ...
          referenceId:   inv.id,
          referenceType: 'savingsVault_investment',
          note:          `Tất toán SavingsVault — lãi: ${profit.toFixed(2)}`,
        },
      });
    });
