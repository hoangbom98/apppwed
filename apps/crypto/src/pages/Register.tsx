  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ background: 'var(--cr-bg)' }}>
      <div className="px-6 pt-16 pb-10 text-center" style={{ background: 'var(--cr-card-bg)' }}>
        ...
        <p className="text-white/70 text-sm mt-1">Tạo tài khoản miễn phí</p>
      </div>

      <div className="flex-1 px-6 pt-8">
        <h2 className="text-xl font-bold mb-6" style={{ color: 'var(--cr-text)' }}>Đăng ký</h2>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="text-sm font-medium block mb-1.5" style={{ color: 'var(--cr-muted)' }}>Họ và tên</label>
            <input
        ...
        <p className="text-center text-sm mt-6" style={{ color: 'var(--cr-muted)' }}>
          Đã có tài khoản?{' '}
          <Link to="/login" className="font-semibold" style={{ color: 'var(--cr-primary)' }}>Đăng nhập</Link>
        </p>
      </div>
    </div>
  );
