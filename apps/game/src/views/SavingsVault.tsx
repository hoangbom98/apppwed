/**
 * SavingsVault.tsx — Savings / Money Market (余额宝)
...
import { getSavingsVaultProducts, getMySavingsVault, investSavingsVault } from '@/api/savingsVault';
...
function InvestForm({ product, onClose }: InvestFormProps) {
  ...
  const mut = useMutation({
    mutationFn: investSavingsVault,
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['my-savingsVault'] });
...
export default function SavingsVault() {
  ...
  const { data: holdings = [], isLoading: hLoading } = useQuery({
    queryKey: ['my-savingsVault'],
    queryFn:  getMySavingsVault,
