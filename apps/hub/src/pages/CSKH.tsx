// @ts-nocheck
// apps/hub/src/pages/CSKH.tsx
import React from 'react';
import { useQuery } from '@tanstack/react-query';
import { CskhPage } from '@lkvip/ui';
import { api } from '@lkvip/ui';
import { hubCskhConfig } from '../configs/cskh.config';
import toast from 'react-hot-toast';

export default function CSKH() {
  const { data: remoteConfig } = useQuery({
    queryKey: ['cskh-config', 'hub'],
    queryFn: () => api.get('/admin/cskh/hub').then(r => r.data?.data ?? r.data),
    staleTime: 5 * 60_000,
    retry: false,
  });

  const config = remoteConfig ?? hubCskhConfig;
