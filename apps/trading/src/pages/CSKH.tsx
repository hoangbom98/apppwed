// @ts-nocheck
// apps/trading/src/pages/CSKH.tsx
import React from 'react';
import { useQuery } from '@tanstack/react-query';
import { CskhPage, api } from '@lkvip/ui';
import { tradeCskhConfig } from '../configs/cskh.config';
import toast from 'react-hot-toast';

export default function CSKH() {
  const { data: remoteConfig } = useQuery({
    queryKey: ['cskh-config', 'trade'],
    queryFn: () => api.get('/admin/cskh/trade').then(r => r.data?.data ?? r.data),
    staleTime: 5 * 60_000,
    retry: false,
  });

  const config = remoteConfig ?? tradeCskhConfig;
