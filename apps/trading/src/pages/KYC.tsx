...
const STATUS_MAP: Record<string, { label: string; cls: string }> = {
  pending:  { label: 'Chưa / Đang chờ duyệt', cls: 'text-yellow-400' },
  approved: { label: 'Đã xác minh',            cls: 'text-green-400' },
...
export default function KYC() {
  ...
  // authController.me returns: { success, data: { id, email, ..., kycStatus } }
  const { data: profile } = useQuery({
    queryKey: ['tradeProfile'],
    queryFn:  () => api.get('/trade/auth/me').then(r => r.data?.data ?? r.data),
  });

  // Fetch KYC record separately to get rejection reason
  const { data: kycRecord } = useQuery({
    queryKey: ['kycRecord'],
    queryFn:  () => api.get('/trade/kyc').then(r => r.data?.data ?? r.data),
    enabled:  !!(profile),
  });
      ...
      <div className="py-8 px-4 flex items-center justify-center min-h-[60vh]">
        <div className="w-full max-w-md rounded-2xl p-7 text-center space-y-4 bn-surface">
          ...
          {kycRecord?.reviewedAt && (
            <p className="text-xs bn-muted">
              Ngày duyệt: {new Date(kycRecord.reviewedAt).toLocaleDateString('vi-VN')}
            </p>
          )}
        </div>
      </div>
  ...
  // ── Pending — kyc record exists (submitted, waiting review) ─────────────────
  // kycStatus stays "pending" after submit until admin approves/rejects
  if (kycStatus === 'pending' && kycRecord) {
    return (
      <div className="py-8 px-4 flex items-center justify-center min-h-[60vh]">
        <div className="w-full max-w-md rounded-2xl p-7 text-center space-y-4 bn-surface">
  ...
  return (
    <div className="py-8 px-4">
      <div className="w-full max-w-lg mx-auto space-y-6">
        ...
        {/* Steps guide */}
        <div className="rounded-2xl p-5 space-y-2 bn-surface">
          {[
        ...
        {/* Form */}
        <form onSubmit={handleSubmit} className="rounded-2xl p-6 space-y-5 bn-surface">
          {/* Full name */}
          <div>
