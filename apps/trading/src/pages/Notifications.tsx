        ...
        {isLoading ? (
          <div className="space-y-3">
            {Array.from({ length: 5 }).map((_, i) => (
              <div key={i} className="bn-surface rounded-2xl p-4 animate-pulse">
                <div className="h-4 rounded w-3/4 mb-2" style={{ background: 'var(--bn-bg-hover)' }} />
                <div className="h-3 rounded w-1/2" style={{ background: 'var(--bn-bg-elevated)' }} />
              </div>
            ))}
          </div>
        ) : filtered.length === 0 ? (
          <div className="bn-surface rounded-2xl p-10 text-center">
          ...
          <div className="space-y-2">
            {filtered.map((notif: any) => (
              <div
                ...
                className={['bn-surface rounded-2xl p-4 flex gap-3 cursor-pointer transition-colors', !notif.isRead ? 'border-l-2 border-blue-500' : ''].join(' ')}
                style={{ cursor: 'pointer' }}
                onMouseEnter={e => (e.currentTarget as HTMLElement).style.background = 'var(--bn-bg-elevated)'}
                onMouseLeave={e => (e.currentTarget as HTMLElement).style.background = 'var(--bn-bg-surface)'}
              >
                {/* Icon */}
