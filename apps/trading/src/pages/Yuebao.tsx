...
import {
  getYuebaoProducts,
  getMyYuebaoInvestments,
  investYuebao,
} from '@/api/trade';
import { useAuthStore } from '@/store/authStore';
import { fmt, fmtTime } from '@/utils/formatters';
import type { YuebaoProduct } from '@/types';
...
export default function YuebaoPage() {
  const { user } = useAuthStore();
  const qc = useQueryClient();
  const [tab, setTab]           = useState<'products' | 'my'>('products');
  const [selected, setSelected] = useState<YuebaoProduct | null>(null);
  ...
  const { data: myData, isLoading: myLoading } = useQuery({
    queryKey: ['yuebao-my'],
    queryFn:  () => getMyYuebaoInvestments(),
    enabled:  !!user && tab === 'my',
  });
  ...
  const investMut = useMutation({
    mutationFn: (vars: { productId: string; amount: number }) => investYuebao(vars),
    onSuccess: () => {
      setMsg('Gửi tiết kiệm thành công!');
      setAmount('');
      setSelected(null);
      qc.invalidateQueries({ queryKey: ['yuebao-my'] });
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-5">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl font-black text-white">Yuebao – Tiết kiệm linh hoạt</h1>
