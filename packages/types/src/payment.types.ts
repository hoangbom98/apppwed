...
/** One entry from the VietQR bank list (mirrors packages/constants/src/banks.js). */
export interface VNBank {
  ...
  shortName:          string;
  logo:               string;
  transferSupported:  number;   // 1 = supports QR transfer
  lookupSupported:    number;   // 1 = account lookup API available
}
...
export interface IVietQRBankListResponse {
  code: string;
  desc: string;
  data: IVietQRBank[];
}
