...
# Run first-deploy
bash source/scripts/first-deploy.sh
```
...
```bash
bash source/scripts/ssl-setup.sh
...
bash source/scripts/deploy.sh --auto-rollback
...
The Nginx config is at `source/nginx/nginx.conf`. It configures 7 server blocks:
...
ls source/backend/node_modules/.prisma/
```
...
grep REDIS_URL source/backend/.env
```
