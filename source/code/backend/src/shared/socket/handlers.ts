...
function setupSocketHandlers(io) {
  ...
  adminNsp.on('connection', (socket) => {
    ...
    // ── Admin: cron job status broadcast ────────────────────────────────────────
    // Emitted by cronController after run/toggle to update all admin dashboards
    socket.on('admin:cron_trigger', (payload) => {
      const role = socket.user?.role;
      if (role !== 'admin' && role !== 'super_admin') return;
      adminNsp.to('admin:all').emit('admin:cron_status', payload);
    });
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('disconnect', () => {
      ...
      if (socket.user?.id && joinedProject) {
        ...
        sessionSvc.countOnline(joinedProject).then((count: number) => {
          io.to(`project:${joinedProject}`).emit('admin:online_count', { project: joinedProject, count });
          io.to('admin:all').emit('admin:online_count', { project: joinedProject, count });
          const adminNsp = io.of('/admin');
          if (adminNsp) {
            adminNsp.to(`project:${joinedProject}`).emit('admin:online_count', { project: joinedProject, count });
            adminNsp.to('admin:all').emit('admin:online_count', { project: joinedProject, count });
          }
        }).catch(() => {});
      }
    });
