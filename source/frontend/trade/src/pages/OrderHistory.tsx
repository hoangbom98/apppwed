...
const STATUS_CONFIG = {
  open:      { label: 'Đang mở',   icon: Clock,          color: 'text-yellow-400 bg-yellow-950/60 border-yellow-900/50' },
...
export default function OrderHistoryPage() {
  ...
  const { data, isLoading, refetch } = useQuery({
    ...
    queryFn: () => getOrders(filter !== 'all' ? { status: filter } : {}),
    enabled: !!user,
  });
      ...
      <div className="flex gap-2">
        {(['all','open','filled','cancelled'] as FilterStatus[]).map(s => (
          ...
          >
            ...
            {s === 'open' && orders.filter((o:any) => o.status === 'open').length > 0 && (
              <span className="ml-1.5 px-1.5 py-0.5 bg-yellow-500 text-black rounded-full text-[10px]">
                {orders.filter((o:any) => o.status === 'open').length}
              </span>
            )}
          </button>
        ))}
