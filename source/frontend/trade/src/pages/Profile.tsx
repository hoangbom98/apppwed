...
import { getMe, getKycStatus, getOrders } from '@/api/trade';
import { useAuthStore } from '@/store/authStore';
import { Shield, ShieldCheck, ShieldAlert, Phone, Mail, LogOut, Settings, Bell, KeyRound } from 'lucide-react';
import { Link } from 'react-router-dom';

const KYC_CONFIG = {
  pending:   { label: 'Chờ duyệt',   icon: ShieldAlert,  color: 'text-yellow-400 bg-yellow-950/50 border-yellow-900/50' },
  approved:  { label: 'Đã xác minh', icon: ShieldCheck,  color: 'text-green-400 bg-green-950/50 border-green-900/50' },
  rejected:  { label: 'Từ chối',     icon: ShieldAlert,  color: 'text-red-400 bg-red-950/50 border-red-900/50' },
  none:      { label: 'Chưa xác minh', icon: Shield,     color: 'text-gray-400 bg-gray-800/50 border-gray-700/50' },
} as const;
...
export default function ProfilePage() {
  ...
  const { data: kycData }    = useQuery({ queryKey: ['kyc'], queryFn: getKycStatus, enabled: !!user });
  ...
  const profile  = meData?.user ?? meData ?? user;
  const kycStatus = (kycData?.status ?? 'none') as keyof typeof KYC_CONFIG;

  const totalOrders  = ordersAll?.data?.length  ?? ordersAll?.total  ?? '—';
  const filledOrders = ordersFill?.data?.length ?? ordersFill?.total ?? '—';
