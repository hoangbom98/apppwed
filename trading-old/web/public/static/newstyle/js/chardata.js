...
function getdata(pid) {
    ...
    $.get(url, function(data) {
        ...
        newprice = data.Price;
        // 
        var close = $("#rate").attr('colse');
        var ps=Number((((data.Price-close)/close)*100)).toFixed(1);
        $('#rate').text(ps+"%")
...
// });
// 订单确认 选择金额
$(".moneys span").click(function () {
    $(this).addClass('active').siblings().removeClass('active');
    // $("#selectmoney").text($(this).text())   // 保存金额到变量
    // $("#yqsy").text($(this).text() * (1 + bili)) // 计算预期收益
    
    order_price = $(this).attr('data-price');
    $('#selectmoney').text(' ' + order_price);
    if (my_money < order_price) {
        // $('.no-money').removeClass('ng-hide');
        layer.msg('Your balance is insufficient');
    } else {
        // $('.no-money').addClass('ng-hide');
    }
    // $('.no-min').addClass('ng-hide');
...
function toggle_order_confirm_panel(type) {
    ...
    if (type == 'lookup') {
        var typename = 'ซื้อ / Buy';
        order_type = 0;

        $('.order_type').removeClass('fail');
        $('.order_type').addClass('rish');
    } else {
        var typename = 'ขาย / Sell';
        order_type = 1;

        $('.order_type').addClass('fail');
        $('.order_type').removeClass('rish');
    }
...
function addorder() {
    ...
    if (order_price > my_money) {
        layer.msg('Balance not enough');
        return;
    }
    if (order_price < order_min_price) {
        layer.msg('Money should be larger than' + order_min_price);
        return;
    }
    if (order_price > order_max_price) {
        layer.msg('Money should be less than' + order_max_price);
        return;
    }
    ...
    $.post(posturl, postdata, function(resdata) {
        ...
        if (resdata.type == 1) {
            layer.msg('successfully ordered',{time:1000},function(){
                window.location.href="/index/user/hold"
            });
