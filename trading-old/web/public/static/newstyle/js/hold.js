            ...
            html+='<div class="item">\
                    <div>\
                        <div class="left">\
                            <div>\
                                <span>' + v.ptitle +'</span>\
                                <span class="'+ ostyle_class2 +'">' + ostyle_name + '（' + v.fee + '）</span>\
                            </div>\
                            <p><span>'+ v.buyprice +'</span> - <span class="' + closeprice_class + '">' + newprice +'</span></p>\
                            <p>' + getLocalTime(v.buytime) + '</p>\
                        </div>\
                        <div class="right">\
                            <p class="' + closeprice_class +'">'+ closeprice +'</p>\
                            <p>'+ formatSeconds2(_end_time) + '</p>\
                        </div>\
                    </div>\
                    <div class="line"><span style="width: ' + baifenbi + '%; transition-duration: 1s;"></span></div>\
                </div>';
            // console.log(html);
            $('.orderlist .slider-left').html(html);
...
function change_category(type) {
    ...
    if (type == 0) {
        ...
        $('.slider-right').css('transform', 'translate(110%, 0px) translateZ(0px)');
    ...
    if (type == 1) {
        ...
        is_ajax_list = 0;
        orderedlist();
        $('.slider-left').css('transform', 'translate(-110%, 0px) translateZ(0px)');
            ...
            html+='<div class="item">\
                    <div>\
                        <div class="left">\
                            <div>\
                                <span>' + v.ptitle +'</span>\
                                <span '+ closeprice_class +'>' + ostyle_name + '（' + v.fee + '）</span>\
                            </div>\
                            <p><span>'+ v.buyprice +'</span> - <span class="' + closeprice_class + '">' + v.sellprice +'</span></p>\
                            <p>' + getLocalTime(v.buytime) + '</p>\
                        </div>\
                        <div class="right">\
                            <p class="' + closeprice_class +'">'+ cc +'</p>\
                            <p>'+ getLocalTime(v.selltime) + '</p>\
                        </div>\
                    </div>\
                </div>';
        })
        if (types == 0) {
            $('.orderlist .slider-right').html(html);
        } else {
            $('.orderlist .slider-right').append(html);
        }
        html = '';
        page++;
        is_ajax_list = 0;
